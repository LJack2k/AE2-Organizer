...
**`ViewerSync`** is the JEI-free bridge the UI talks to (so the core never imports JEI). The **`@JeiPlugin`** (`TerminalOrganizerJeiPlugin`) registers two GUI handlers and stays dormant if JEI is absent:
