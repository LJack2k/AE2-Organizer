 ...
 * present (see {@link RsLegacyBackend#theme()}), so referencing RS's wrench item here is safe.
 */
public final class RsLegacyTheme implements Theme {

    public static final RsLegacyTheme INSTANCE = new RsLegacyTheme();

    private RsLegacyTheme() {}
    ...
    /** Refined Storage's own wrench item (RsLegacyTheme only loads when RS is present). */
    private static ItemStack wrench() {
        if (wrench == null) {
            var item = BuiltInRegistries.ITEM
                    .getOptional(new ResourceLocation("refinedstorage", "wrench"))
