 ...
 * <p>
 * <b>F6 cycles the GUI scale</b> (auto → 1 → 2 → 3 → 4 → auto). Vanilla has no
 * keybind for this, and Video Settings is several clicks away with the terminal
 * closed in between -- which is the awkward part, because the interesting case is
 * changing scale <em>while a storage screen is open</em>, so the open screen is
 * re-inited through {@code resize()} rather than constructed fresh.
 * <p>
...
public final class DevKeybinds {
    ...
    /** auto, then the usable scales; {@code resizeDisplay} clamps anything the window can't take. */
        ...
        if (mc.gui != null) {
            mc.gui.setOverlayMessage(Component.literal(
                    "GUI scale: " + (next == 0 ? "auto" : next)
                            + " (effective " + (int) mc.getWindow().getGuiScale() + ")"), false);
        }
    }
}
